...
JFrog tenant** — nothing to install, nothing shared with your neighbors. Work top to bottom;
each lab stands alone if you fall behind during the live session.
...
> **your** numbers are the ones Xray shows on your tenant. The gap is the point, not the
> digits.
...
3. Your card also carries your **JFrog tenant** URL and login — open it in a second browser
   tab. You are the admin of that whole tenant: repos, Xray, policies — all yours today.
...
Your tenant is already wired into the seat (`$JF_HOST` is set in your shell; Docker is logged
in). Confirm the front door works by pulling a Chainguard image **through** your tenant's
remote repo instead of straight from the internet:
...
visible to Xray**. In the tenant UI: Artifactory → Artifacts → `cgr` — there's your pull,
already indexed.
...
```bash
docker build -t $JF_HOST/dev-docker/octoshop-web:v1 .
docker push $JF_HOST/dev-docker/octoshop-web:v1
docker build -t $JF_HOST/dev-docker/octoshop-web:v2 -f Dockerfile.cg .
docker push $JF_HOST/dev-docker/octoshop-web:v2
```

Now the money shot. In the tenant UI: **Xray → Scans List** (or Artifactory → `dev-docker` →
select the artifact → Xray tab). Give indexing a minute, then put v1 and v2 side by side:
...
```bash
docker build -t $JF_HOST/dev-docker/octoshop-web:v3 -f Dockerfile.cg .
docker push $JF_HOST/dev-docker/octoshop-web:v3
```
...
docker build -t $JF_HOST/dev-docker/octoshop-web:v5 -f Dockerfile.cg .
docker push $JF_HOST/dev-docker/octoshop-web:v5
```
...
docker build -t $JF_HOST/dev-docker/octoshop-web:v5-fixed -f Dockerfile.cg .
docker push $JF_HOST/dev-docker/octoshop-web:v5-fixed
```
...
A dashboard informs; a **policy** enforces. In the tenant UI:

1. **Xray → Watches & Policies → New Policy**: name `block-high`, type Security, rule: minimal
   severity **High** → actions: **Block Download**.
2. **New Watch**: name `block-high-watch`, resources: `dev-docker` + `prod-docker`, assign
   policy `block-high`. Prod is the governed endpoint — the cluster you'll deploy from in
   Lab 5 drinks from it, so a violating artifact must be unservable there, full stop.
   Create both as **global** objects — if the UI drops you into a Project context, back out:
   project-scoped watches behave differently and the guide assumes global.
...
3. **Apply it to what's already there**: your v1/v3 images were scanned *before* this watch
   existed, and watches only judge new scan events. In the watch list: row menu (⋮) →
   **Apply on Existing Content** → pick a range covering today → run. This back-fills
   violations for everything already in the repos — and enforcement trails it by a few
   minutes, so **verify the gate is armed before the demo**:

   ```bash
   curl -s -o /dev/null -w '%{http_code}\n' -u <you>:<your-jfrog-password> https://$JF_HOST/v2/dev-docker/octoshop-web/manifests/v3
   ```
...
Now promote the clean build to production (the CLI is pre-configured as `lab`):

```bash
jf rt docker-promote octoshop-web dev-docker prod-docker --copy --source-tag v2 --target-tag v2-mine
docker pull $JF_HOST/prod-docker/octoshop-web:v2-mine
```

The clean build flows. Now try the poisoned one:

```bash
jf rt docker-promote octoshop-web dev-docker prod-docker --copy --source-tag v3 --target-tag v3
docker rm -f $(docker ps -aq) 2>/dev/null; docker rmi -f $(docker images -aq) 2>/dev/null
docker system prune -af
docker pull $JF_HOST/dev-docker/octoshop-web:v3
```
...
downloaded due to the download blocking policy configured in Xray for dev-docker.
```
...
you just wrote. No memo, no training: the unsafe path simply stops working. (Sound familiar?
...
*This lab works even if you skipped 2–4: a known-good build is already in your `prod-docker`,
pushed at seed time.*
...
```bash
kubectl set image deploy/web web=$JF_HOST/dev-docker/octoshop-web:v2 -n octoshop
...
> `prod-docker` only. One permission target turns "please only deploy approved images" from a
...
*Seat misbehaving? `sudo /opt/lab/bootstrap.sh` rebuilds the cluster side in ~90 s. Tenant
misbehaving? You're the admin — but the facilitator has a spare tenant, like a spare seat.*
